...
The most stubborn artifacts — `}` for `y` (337 instances), `3'ou` for `you`, `v/` for `w` — are _character-level substitutions_ specific to how Tesseract misreads particular typefaces. The 1833 Kercheval book uses a decorative typeface where Tesseract consistently reads `y` as `}`, `w` as `v/`, and `y` as `3'`. These aren't random errors; they're systematic misreadings of specific glyphs.
...
We ran out of API credits ($0 wallet, $100 limit — the limit is how much you _can_ spend, not how much you _have_). Before adding funds and re-running:
...
The rate limit problem was never going to be solved by retries alone. The built-in `RetryRateLimitHandler` (added in neo4j-graphrag 1.14.0, [issue #295](https://github.com/neo4j/neo4j-graphrag-python/issues/295)) backs off _after_ a 429 — but with Haiku's free tier limits (10K input tokens/min, 50 requests/min), you blow past the ceiling before a single retry fires. The pipeline launches all chunk extractions concurrently. You need throttling, not just retry.
...
What the SDK _doesn't_ do is proactive throttling — it retries after getting a 429 rather than preventing them. For the free tier (50 req/min), you burn through your quota before a single retry fires. We ported the `ThrottledAnthropicLLM` pattern from the SimpleKGPipeline work into a `ThrottledClient` wrapper that serializes requests with a minimum interval:
...
### Apr 13th Update (2 weeks later): Implement Vector and Hybrid RAG

Implemented vector search with Claude Code. It works but needs updates:

- It calls out to huggingface API directly for embedding. I don't have a HF account linked or APIkey. So I'm assuming this could probably be done faster? Currently using nomic-embed-v2-moe

- Example POST /query request that uses both semantic vector (`question`) and graph traversal (`filters`). I am not exactly sure what the purpose of `mode` is, because I think we're determining it from the presence/absence of filters and questions

```json
{
  "filters": ["place-shenandoah-valley"], // array of entity ids (name) to filter by
  "k": 10,
  "mode": "graph", // graph for graph and hybrid search, naive for vector only similarity
  "question": "what were the most populous tribes during the contact period in western frontier of virginia" // this gets embedded to pull similar chunks from the vector index
}
```

For testing, we also added /embed-locally endpoint to get nomic embeddings for testing in neo4j locally.
